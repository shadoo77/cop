...
export function stringEncryptor(str: string) {
  try {
    ...
    const encryptedString =
      encryptor.update(str, 'utf8', 'hex') + encryptor.final('hex');
...
export function stringDecryptor(encryptedString: string) {
  try {
    const encryptedText = Buffer.from(encryptedString, 'base64').toString(
      'utf-8'
    );
    const decryptor = crypto.createDecipheriv(algorithm, key, iv);
    return (
      decryptor.update(encryptedText, 'hex', 'utf8') + decryptor.final('utf8')
    );
  } catch (err) {
